...
#         else:
    ...
    def coupling_matrix(self,win,wig_3j_12):
        return np.dot(wig_3j_12,win*(2*self.window_l+1))/4./np.pi #FIXME: check the order of division by l.
...
#         print(m1m2[0],self.wig_3j)
        wig_3j_12=self.wig_3j[m1m2[0]]
        wig_3j_12=wig_3j_12*self.wig_3j[m1m2[1]]
        ...
        if len(indxs)>2:
            ...
            wig_3j1324=self.wig_3j[m1m2]
            m1m2=cov_m1m2s(corrs=(corr[1],corr[3]))
            wig_3j1324=wig_3j1324*self.wig_3j[m1m2]

            m1m2=cov_m1m2s(corrs=(corr[0],corr[3]))
            wig_3j1423=self.wig_3j[m1m2]
            m1m2=cov_m1m2s(corrs=(corr[1],corr[2]))
            wig_3j1423=wig_3j1423*self.wig_3j[m1m2]
        ...
        if not do_cov:
            ...
            win['M']=self.coupling_matrix(win['cl'], wig_3j_12)*(2*self.l[:,None]+1) #FIXME: check ordering
        ...
        if do_cov:
            ...
            win['M1324']=self.coupling_matrix(win['cl1324'], wig_3j1324) #/np.gradient(self.l)
            win['M1423']=self.coupling_matrix(win['cl1423'], wig_3j1423) #/np.gradient(self.l)

#             win['M1324']/=self.f_sky**3 #FIXME: Where is this factor coming from
#             win['M1423']/=self.f_sky**3
        return win

    def set_window(self,corrs=None,corr_indxs=None):
        if self.Win is None:
            self.Win={'cl':{},'xi':{}}
