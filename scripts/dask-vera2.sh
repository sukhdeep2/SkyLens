...
WORKER_MEM=75000MB #$(($MEMORYLIMIT / $NPROCS_worker_node))MB
echo 'memory ' $WORKER_MEM $MEMORYLIMIT

scheduler_ncpu=2
NPROCS_scheduler_node=1  #$(($NCPU -$scheduler_ncpu ))
...
# env|grep SLURM
srun -w $_hostname -O -N 1 -n 1 --cpus-per-task=$scheduler_ncpu --cpu-bind=none --mem-bind=none --unbuffered dask-scheduler --scheduler-file=$SCHEFILE \
...
srun -w $_hostname -O -N 1 -n 1 --cpus-per-task=$NPROCS_scheduler_node --cpu-bind=none --mem-bind=none --unbuffered dask-worker --scheduler-file=$SCHEFILE \
        --nprocs $NPROCS_scheduler_node --nthreads $NTHREADS --local-directory=$WORKSPACE --memory-limit $WORKER_MEM &

srun -x $_hostname -N $NWORKER -n $NWORKER --cpu-bind=none --mem-bind=none --unbuffered dask-worker --scheduler-file=$SCHEFILE \
