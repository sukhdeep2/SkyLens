...
if __name__=='__main__':
    ...
    print(use_complicated_window,unit_window,lognormal,do_blending,do_SSV_sim,use_shot_noise,args.noise,np.bool(args.noise),not args.noise,Scheduler_file)
    ...
    galaxy_zbins=lsst_source_tomo_bins(zp=np.array([z0_galaxy]),ns0=10,use_window=use_window,nbins=1,
                                  scheduler_info=scheduler_info,n_zs=n_zs,
    ...
    z0_shear=1 #1087
    shear_zbins=lsst_source_tomo_bins(zp=np.array([z0_shear]),ns0=30,use_window=use_window,n_zs=n_zs,nbins=1,
                                        window_cl_fact=window_cl_fact*(1+ww*use_complicated_window),
...
#         cl0_win['cov'][corr]=client.compute(clG_win['cov'][corr+corr]).result()[0]['final_b']
    ...
    cl_sim_W=Sim_jk(nsim=nsim,do_norm=False,njk=njk,kappa0=kappa0,jkmap=jkmap,skylens_kwargs=Skylens_kwargs,
                    kappa_class=kappa_win,use_shot_noise=use_shot_noise,use_cosmo_power=use_cosmo_power, #,fsky=f_sky
                    nside=nside,lognormal=lognormal,lognormal_scale=lognormal_scale,
