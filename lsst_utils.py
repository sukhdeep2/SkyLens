...
def source_tomo_bins(zp=None,p_zp=None,nz_bins=None,ns=26,ztrue_func=None,zp_bias=None,
    ...
    for i in np.arange(nz_bins):
        ...
        zs_bins['SN']['shear'][:,i,i]=shear_shape_noise_calc(zs1=zs_bins[i],zs2=zs_bins[i],sigma_gamma=sigma_gamma)
...
def galaxy_tomo_bins(zp=None,p_zp=None,nz_bins=None,ns=10,ztrue_func=None,zp_bias=None,window_cl_fact=None,mag_fact=0,
    ...
    if use_window:
        zg_bins=set_window(zs_bins=zg_bins,f_sky=f_sky,nside=nside,mask_start_pix=mask_start_pix,window_cl_fact=window_cl_fact,
                           )
    return zg_bins
...
def cmb_bins(zs=1100,l=None):
    ...
    zs_bins=zbin_pz_norm(zs_bins=zs_bins,bin_indx=0,zs=np.atleast_1d(zs),p_zs=np.atleast_1d(1),ns=0,bg1=1)
