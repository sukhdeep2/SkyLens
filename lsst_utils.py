...
def lsst_pz_source(alpha=1.24,z0=0.51,beta=1.01,z=[]):
    p_zs=z**alpha*tc.exp(-(z/z0)**beta)
    p_zs/=(tc_gradient(z)*p_zs).sum()
    return tc.tensor(p_zs,dtype=tc.double)
...
def ztrue_given_pz_Gaussian(zp=[],p_zp=[],bias=[],sigma=[],zs=None,ns=0):
    ...
    """
    if zs is None:
        zs=tc.linspace(min(zp-sigma*5),max(zp-sigma*5),500,dtype=tc.double)

    y=tc.tensor(np.tile(zs, (len(zp),1)  )) #FIXME
    pdf=tc.tensor(gaussian.pdf(y.transpose(1,0),loc=zp+bias,scale=sigma),dtype=p_zp.dtype)#.transpose(1,0)

    dzp=tc_gradient(zp)
    p_zs=(dzp*p_zp*pdf).sum(1)
...
def source_tomo_bins(zp=None,p_zp=None,nz_bins=None,ns=26.,ztrue_func=None,zp_bias=None,
    ...
    if nz_bins is None:
        nz_bins=1
    z_bins=tc.linspace(min(zp)-0.0001,max(zp)+0.0001,nz_bins+1)

    if zs is None:
        zs=tc.linspace(0,2.5,100,dtype=tc.double)
    dzs=tc_gradient(zs)
    dzp=tc_gradient(zp) if len(zp)>1 else tc.tensor([1],dtype=tc.double) #FIXME
    zp=tc.tensor(zp,dtype=tc.double)

    zl_kernel=tc.linspace(0,2,50)
    lu=Lensing_utils()
    cosmo_h=cosmo_h_PL

    for i in tc.arange(nz_bins):
        i=i.item()
        zs_bins[i]={}
        indx=np.searchsorted(zp,z_bins[i:i+2]) #search for nearest values to bin edges
        ...
        if ztrue_func is None:
            ...
            print(ns,p_zs,dzp)
            nz=(ns*p_zs*dzp[indx[0]:indx[1]]).type(tc.double)
            print(indx,zs,z_bins)
        else:
            ns_i=ns*(p_zp[indx[0]:indx[1]]*dzp[indx[0]:indx[1]]).sum()
        ...
        x= p_zs>1.e-10
        zs_bins[i]['z']=zs[x]
        zs_bins[i]['dz']=tc_gradient(zs_bins[i]['z']) if len(zs_bins[i]['z'])>1 else 1
        zs_bins[i]['nz']=nz[x]
        zs_bins[i]['W']=1.
        zs_bins[i]['pz']=p_zs[x]*zs_bins[i]['W']
        zs_bins[i]['pzdz']=zs_bins[i]['pz']*zs_bins[i]['dz']
        zs_bins[i]['Norm']=zs_bins[i]['pzdz'].sum()
...
def lens_wt_tomo_bins(zp=None,p_zp=None,nz_bins=None,ns=26,ztrue_func=None,zp_bias=None,
    ...
    z_bins=tc.linspace(min(zp),max(zp)*0.9,nz_bins) if z_bins is None else z_bins
